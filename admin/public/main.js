...
let url = "https://sleepy-tundra-34734.herokuapp.com/api/recipes"; // Ny VUe-instans
...
let app = new Vue({
  ...
  methods: {
    get: function () {
      // Hämta alla recept med fetch api
      ...
      }) // Lagra datan i recipeList i Vue:s data
    ...
    add: function (event) {
      ...
      }).then(res => res.json()).then(data => console.log(data)).catch(err => console.log(err)); // Gör textsträngar av ingrediens- och instruktions-array igen
    ...
    upd: function (event) {
      ...
      let ob = {}; // Lagra värden från arrayen som skapats i ett objekt
    ...
    readMore: function (event) {
      // Möjliggör att fälla ned och upp boxarna med formulären frö att visa mer/mindre
